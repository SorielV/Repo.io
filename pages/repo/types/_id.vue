...
export default {
  ...
  async asyncData({ app, query, params: { id: idCatalog } }) {
    ...
    try {
      const {
        data: { data: catalog }
      } = await app.$axios.get('/api/catalog/type/' + idCatalog)

      return {
        baseRouter: '/repo/types/' + idCatalog,
        catalog,
        query: queryParams
      }
    } catch (error) {
  ...
  async created() {
    ...
    try {
      ...
      const types = [this.catalog].reduce((arr, { id: idCatalog, value }) => {
