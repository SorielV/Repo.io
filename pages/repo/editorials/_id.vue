<template lang="pug">
  section
    .type-head(style="background: black;")
      figure.image(style="max-height: inherit;")
        img(:src="catalog.image || '/public/empty.webp'" style="max-height: inherit; background: black;")
          ...
          nav.panel.has-background-white(style="border-radius: 6px")
              ...
              p.panel-tabs
                ...
                b-tabs(v-model='filterTab' expanded)
                  // Catalogo Recursos
                  b-tab-item(label='Recurso' disabled)
  ...
  async asyncData({ app, query, params: { id: idCatalog } }) {
    ...
    */
    ...
    try {
      const {
        data: { data: catalog }
      } = await app.$axios.get('/api/catalog/author/' + idCatalog)

      return {
        baseRouter: '/repo/authors/' + idCatalog,
        catalog,
        query: queryParams
      }
    } catch (error) {
