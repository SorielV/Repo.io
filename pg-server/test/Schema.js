module.exports = function Schema(
    ...
    for (let key in Schema) {
      if (!Schema[key].required) {
        ...
        if (!data.hasOwnProperty(key)) {
          if (!Schema[key].default) throw new Error(`${key} Faltante`)
  ...
  Model.prototype.save = async function() {
    ...
    const stringifyType = this.constructor.stringifyType

    for (let key in model) {
      columnNames.push(`"${key}"`)
      columnValues.push(
        model[key] !== null
          ? stringifyType(model[key], this.Schema[key].type)
          : 'default'
      )
    }
  ...
  Model.prototype.update = async function() {
    ...
    return statement + ' ' + whereStatement.join(' and ') + ' RETURNING *'
  }
  ...
  Model.stringifyType = function(value, type) {
  ...
  Model.valitation = function(data) {
    ...
    const formatType = instance ? this.formatType : Model.formatType
    ...
    for (let key in Schema) {
      if (!Schema[key].required) {
        if (data[key])
          model[key] =
            data[key] === undefined
              ? formatType(data[key], Schema[key].type)
              : null
        ...
        if (data[key] === undefined) throw new Error(`${key} Faltante`)
          ...
          model[key] =
            data[key] !== null ? formatType(data[key], Schema[key].type) : null
      }
    }
  ...
  Model.find = async function(options = {}) {
    const { where = null, columns = [] } = options
    const table = Model.prototype.Table
